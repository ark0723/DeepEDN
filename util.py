...
## save network
def save(ckpt_dir, netG_a2b, netG_b2a, netROI, netD_a, optimG, optimD, optimR, epoch):
    if not os.path.exists(ckpt_dir):
        os.makedirs(ckpt_dir)

    torch.save({'netG_a2b': netG_a2b.state_dict(), 'netG_b2a': netG_b2a.state_dict(),
                'netD_a': netD_a.state_dict(), 'netROI':netROI.state_dict(), 'optimG': optimG.state_dict(), 'optimD': optimD.state_dict(),
                'optimR': optimR.state_dict()},
               "%s/model_epoch%d.pth" % (ckpt_dir, epoch))

## load network
def load(ckpt_dir, netG_a2b, netG_b2a, netROI, netD_a, optimG, optimD, optimR):
    if not os.path.exists(ckpt_dir):
        epoch = 0
        return netG_a2b, netG_b2a, netD_a, netROI, optimG, optimD, optimR, epoch
    ...
    netROI.load_state_dict(dict_model['netROI'])
    optimG.load_state_dict(dict_model['optimG'])
    optimD.load_state_dict(dict_model['optimD'])
    optimR.load_state_dict(dict_model['optimR'])
